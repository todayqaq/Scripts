 ...
 */
 ...
 let En8 = randomE(8);
 let En8Value = randomE(6);
 let En10 = randomE(10);
 let En10Value = randomE(13);
 ...
 function box_info(timeout = 3 * 1000) {
    var initStr = `{
        "mysticTime":${ts},
        "money":1,
        "keyid":"dict-usertask-key",
        "${En8}":"${En8Value}",
        "${En10}":"${En10Value}",
        "yduuid":"13ddd10eeef211a0f44ea8e0a5e7d23a",
        "network":"wifi",
        "ssid":"x1q28f30b621c6b27de",
        "screen":"1080x1920",
        "appVersion":"9.1.18",
        "imei":"13ddd10eeef211a0f44ea8e0a5e7d23a",
        "mid":"5.1.1",
        "keyfrom":"mdict.9.1.18.android",
        "client":"androidPad",
        "abtest":6,
        "vendor":"tencent",
        "newImei":354730110922626,
        "model":"M2007J22C"
        }`
    var jsonStr = JSON.parse(initStr);
    var sorted = {}
        Object.keys(jsonStr).sort().forEach(function(val) {
        sorted[val] = jsonStr[val]
    })
    pointParam = Object.keys(sorted).toString()
    ...
    let _sign = serialize(sorted)+"&key=ttfMFaa7tiPyAc3DanKeIMzEejm"
    let sign = MD5Encrypt(_sign)        
    getUrl = `https://dict.youdao.com/dictusertask/lottery/info?${_sign}&sign=${sign}&pointParam=${pointParam}`
     return new Promise((resolve) => {
         let url = {
             url: `${getUrl}`,
             headers: {
 ...
 function open_box(timeout = 3 * 1000) {
        ...
        let sign = MD5Encrypt(_sign)
        let postUrl = `https://dict.youdao.com/dictusertask/lottery/execute?network=wifi&ssid=x1q28f30b621c6b27de&screen=1080x1920&appVersion=9.1.18&imei=13ddd10eeef211a0f44ea8e0a5e7d23a&mid=5.1.1&keyfrom=mdict.9.1.18.android&client=androidPad&abtest=6&vendor=tencent&newImei=354730110922626&model=M2007J22C&product=mdict`
        let postBoyd = `mysticTime=${ts}&${En8}=${En8Value}&sign=${sign}&keyid=dict-usertask-key&lotteryType=0&${En10}=${En10Value}&yduuid=13ddd10eeef211a0f44ea8e0a5e7d23a&pointParam=${pointParam}`
            return new Promise((resolve) => {
         let url = {
             url: `${postUrl}`,
             headers: {
             ...
             try {
                 ...
                 if (result.code == 0) {
                    if(result.data.goodsType == 5){
                     ...
                     console.log(`\n 开幸运礼盒: ${result.msg} \n`);
                     msg += `开幸运礼盒:${JSON.stringify(result.msg)}\n`
 ...
 function sign(timeout = 3 * 1000) {
    ...
    var jsonStr = JSON.parse(initStr);
        ...
        let _sign = serialize(sorted)+"&key=ttfMFaa7tiPyAc3DanKeIMzEejm"
        let sign = MD5Encrypt(_sign)        
        getUrl = `https://dict.youdao.com/dictusertask/cheese/collect?${_sign}&sign=${sign}&pointParam=${pointParam}`
    return new Promise((resolve) => {
         let url = {
             url: `${getUrl}`,
             headers: {
  ...
  */
  ...
  function tx(){
     ...
     (resolve) => {
        ...
        let _sign = serialize(sorted)+"&key=ttfMFaa7tiPyAc3DanKeIMzEejm"
        let sign = MD5Encrypt(_sign)        
        getUrl = `https://dict.youdao.com/dictusertask/withdraw/execute?${_sign}&sign=${sign}&pointParam=${pointParam}`
        let url = {
             url:`${getUrl}`,
             headers:{
                 'Host':'dict.youdao.com',
         ...
         $.get(url, async (error, response, data) => {
            try {
                ...
                let result = JSON.parse(data);
                   ...
                   console.log(result)
                   msg += `提现一元:${JSON.stringify(result.msg)}\n`
 ...
 function info(){
    return new Promise(
        (resolve) =>{
            ...
            let _sign = serialize(sorted)+"&key=ttfMFaa7tiPyAc3DanKeIMzEejm"
            let sign = MD5Encrypt(_sign)
            getUrl = `https://dict.youdao.com/dictusertask/withdraw/account?${_sign}&sign=${sign}&pointParam=${pointParam}`
            let url = {
                url:`${getUrl}`,
                headers:{
                    'Host':'dict.youdao.com',
               ...
               try {
                   ...
                   if(result.code == 0){
                      ...
                      console.log(result.data.amount)
                        msg += `当前余额：${JSON.stringify(result.data.amount)}\n`
 ...
 async function Envs() {
     if (ydcd_data) {
         if (ydcd_data.indexOf("&") != -1) {
             ...
             //console.log(ydcd_dataArr[0]);
             //console.log(typeof(ydcd_dataArr[0]))
         }
     } else {
  ...
  */
  function randomE(e) {
    e = e || 32;
    var t = "qwertyuiopasdfghjklzxcvbnm",
        a = t.length,
        n = "";
    for (i = 0; i < e; i++)
        n += t.charAt(Math.floor(Math.random() * a));
